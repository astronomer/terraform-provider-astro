...
func TestAcc_DataSourceClusterOptions(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config:      astronomerprovider.ProviderConfig(t, true, false) + clusterOptions("invalid", "AWS"),
				ExpectError: regexp.MustCompile(`type value must be one of`),
			},
			{
				Config: astronomerprovider.ProviderConfig(t, true, false) + clusterOptions("HYBRID", "AWS"),
				Check: resource.ComposeTestCheckFunc(
					checkClusterOptions("AWS"),
				),
			},
			{
				Config: astronomerprovider.ProviderConfig(t, true, false) + clusterOptionsWithoutProviderFilter("HYBRID"),
				Check: resource.ComposeTestCheckFunc(
