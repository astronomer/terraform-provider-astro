...
func deleteWorkspaceOutsideOfTerraform(t *testing.T, name string) {
	...
	resp, err := client.ListWorkspacesWithResponse(ctx, os.Getenv("HOSTED_ORGANIZATION_ID"), &platform.ListWorkspacesParams{
		Names: &[]string{name},
	})
	if err != nil {
		assert.NoError(t, err)
	...
	_, err = client.DeleteWorkspaceWithResponse(ctx, os.Getenv("HOSTED_ORGANIZATION_ID"), resp.JSON200.Workspaces[0].Id)
	assert.NoError(t, err)
}
