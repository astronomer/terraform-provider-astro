...
The clients under `internal/clients/{iam,platform,labs}/api.gen.go` are generated by
[`oapi-codegen`](https://github.com/deepmap/oapi-codegen) from Core's OpenAPI specs. Regenerate them with:

```
make api_client_gen
```

This reads the Core specs from the sibling `astro` checkout (see `CORE_*_OPENAPI_SPEC` in the `Makefile`),
so add new Core fields/endpoints there first. `make api_client_gen` is also where new API tags become
available to the provider — a tag must appear in the relevant `-include-tags` list in the `Makefile` to be
generated (e.g. `NotificationChannels`, `Alerts`).

### Forked union template

`internal/clients/oapi-templates/union.tmpl` is a **fork** of `oapi-codegen`'s built-in `union.tmpl`
(pinned to `DESIRED_OAPI_CODEGEN_VERSION`). It customizes the `From*`/`Merge*` methods to inject the
discriminator value into the marshaled JSON rather than assigning a struct field, so that discriminated
unions whose discriminator is **optional** in Core (rendered by `oapi-codegen` as a pointer/absent Go
field — e.g. the labs `Update*Alert*` family) still compile and serialize the correct `"type"`.

When bumping `DESIRED_OAPI_CODEGEN_VERSION`, diff the new upstream `union.tmpl` and re-apply the
customizations; the template header documents exactly what to re-apply. After regenerating, run
`go build ./...` to confirm the optional-discriminator unions still compile.
