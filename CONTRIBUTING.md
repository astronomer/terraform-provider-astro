...
The clients under `internal/clients/{iam,platform}/api.gen.go` are generated by
...
so add new Core fields/endpoints there first. A new API tag must also appear in the relevant
`-include-tags` list in the `Makefile` to be generated.
...
field — e.g. the alert update requests) still compile and serialize the correct `"type"`.
