...
page_title: "astro_users_list Data Source - astro"
subcategory: ""
description: |-
  Users data source (list variant). Identical to astro_users but returns the users collection as an ordered list instead of a set, for significantly better terraform plan performance on large organizations.
---

# astro_users_list (Data Source)
...
- `dag_roles` (Attributes Set) The DAG roles assigned to the user (see [below for nested schema](#nestedatt--users--dag_roles))
...
<a id="nestedatt--users--dag_roles"></a>
### Nested Schema for `users.dag_roles`
